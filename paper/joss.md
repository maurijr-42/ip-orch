...
Machine Learning Interatomic Potentials (MLIPs) have emerged as a powerful alternative, enabling near ab initio accuracy at a fraction of the computational cost by learning the potential energy surface from DFT reference data `[@kozinsky_2022,@mlips_practical_guide]`. By scaling with the number of atoms rather than electrons, MLIPs allow simulations of larger systems, longer timescales, and more diverse structural environments `[@focassio_2024]`. This has expanded the scope of atomistic modeling to problems such as phase transformations, defect dynamics, and biomolecule–material interactions.
...
This is implemented through a lightweight and modular architecture, as shown in Figure \label{fig:arch}. A central `ModelFactory` creates ASE calculators from simple aliases, while the command-line interface and the `IPOrch` Python API share the same execution layer for environment discovery, model selection, and execution. During runtime, IP-Orch iterates over selected (environment, model) pairs and runs the user workflow inside each target environment using either the environment's Python executable (for local virtual environments) or `conda run` (for named Conda environments), ensuring isolation and avoiding dependency conflicts. Execution can be sequential or concurrent across selected pairs through a bounded parallel worker pool. The worker process also applies runtime safeguards such as model-alias validation, path-based virtual environment resolution, and notebook-safe subprocess settings. By delegating performance to the underlying MLIPs, IP-Orch focuses on providing a transparent and low-friction framework for reproducible benchmarking and comparison across models.
